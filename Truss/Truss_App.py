...
class MainWindow(Ui_TrussStructuralDesign, qtw.QWidget):
    def __init__(self):
        ...
        # Install event filter via controller
    ...
    def eventFilter(self, obj, event):
        if obj == self.controller.view.scene:
            ...
            if et == qtc.QEvent.GraphicsSceneMouseMove:
                ...
                # Use controller to get item at position
                s = self.controller.get_item_at(scenePos, self.gv_Main.transform())
                if s is not None and s.data(0) is not None:
                    strScene += ' (' + s.data(0) + ')'
                # Use controller to get items at position
                items = self.controller.get_items_at(scenePos)
                item_names = [item.name if hasattr(item, 'name') else None for item in items]
                for i in item_names:
                    strScene += ', ' + (i if i is not None else 'none')
                # Use controller to update UI
                self.controller.update_mouse_position(strScene)
